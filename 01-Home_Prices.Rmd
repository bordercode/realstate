...
Metropolitan areas are geographic units that are particularly suited to understand regional real estate dynamics and provide a base scenario to study internal migration patterns within the US territory as well as specific regions such as the US-Mexico border.
...
A primary goal is to explore real estate market fluctuations over time, to achieve this purpose we measure three concepts: house price levels, price appreciation and purchasing parity based on housing prices. 
...
[@bureauofeconomicanalysisPersonalIncomeCounty2023]. This variable contributes to capture inequality and financial barriers to access the housing market. 
...
By analyzing the search volume data obtained from Google Trends, this data source allows us to observe and compare the changes in preferences and search patterns for various topics using a score in the close domain [0-100], where 100 indicates the highest volume of searches for a particular place and time and 0 indicates the lowest volume for the term at a precise moment in time and location.
...
By examining this aggregate economic indicator it is possible to distinguish which states of the country exhibit the most expensive housing markets and therefore which areas of the country represent particular challenges either for households entering the market or those families already paying a mortgage. 


Regional price parities capture variations in the cost of living across different regions of the country, this indicator is important because it helps to understand the relative purchasing power and living standards in different geographic areas and particularly when considering the housing component, it provides a measure to compare the real estate market conditions relative to the country.  
...
Figure 1 shows regional price parities by state relative the average US for 2022.
...
scale_fill_manual(values=c("firebrick","deepskyblue4"), labels=c("Below average","Above average" ), name="")+
...
It can be seen that the US west coast border state of California has the highest housing price parity among the 48 states (excluding Hawaii, Alaska and DC) for 2022. This  pattern suggest financial pressures for residents in west coast states that may influence the current out of state internal migration documented by academic research elsewhere [@blumenbergJobsHousingBalance2021] for states such as California.
...
The data analysis strategy design aims to extract the changes in prices over a two decade time frame for each metropolitan area so, we are able to measure not only which metropolitan areas have registered the bigger price gains between 2000 and 2020 but also the speed of that change, as measured by it´s annual rate of growth. 

Measuring price fluctuations using the HPI developed by an independent federal agency such as FHFA, is important for two reasons in the context of this investigation: I) It informs on the potential changes on home equity fluctuations over time with a geographic perspective, which exposes real estate investment opportunities  II) It reflects existing financial opportunities and/or barriers to entry into the housing market for households looking to relocate, so is an appropriate economic indicator that helps to explain  current internal migration patterns.  
...
mapa<-mapview(df, map.types =  "OpenStreetMap.DE",cex="hpi20", zcol="hpi20", layer.name= "MSA House Price Index",legend = TRUE,homebutton =FALSE)
...
Empirical findings reveal a pronounced surge in housing prices, as gauged by the House Price Index (HPI), within geographically delineated metropolitan areas across the United States over the last two decades. Noteworthy regions exhibiting such trends encompass metropolitan statistical areas (MSAs) situated along the West Coast, the southeastern part of Florida, and the northwestern region. Over the span of the past two decades, these identified regions have witnessed substantial increase in housing values.

Among the states located on the West Coast, the State of California is particularly outstanding with areas of concern such as the Bay area, where the so called Silicon Valley region represents a mayor labor market hub particularly for highly skilled workers associated with the technology sector. This geographic area holds substantial economic importance due to it's concentration of technological innovation, fostering an environment  that attracts and sustains a skilled workforce with above average income, a feature that also translates into relevant tax revenue sources for the entire state. 
...
Using the above estimated pattern it is convenient to determine: which metropolitan areas registered the highest price increments since base year 1995? according to the Federal Housing Finance Agency HPI.
...
**Figure 3 Top 20 Metropolitan areas by HPI increase 2020.**
```{r, include=FALSE}
...
II) Within the US Mexico border, the importance of California state is clear, representing $75\%$ of the border MSA with highest HPI growth. In fact, San Diego-Chula Vista-Carlsbad exhibits the highest annual increase in this border state in 2022 with a $18.55\%$  
II) The real estate market in Florida shows the fastest growing level of prices of the nation, with sharp increases particularly during the Trump administration, with Metropolitan areas such as West Palm Beach, North Port-Sarasota-Bradenton, Naples Marco Island and Tampa featuring double digit increases with a maximum increase of $32.77\%$ the highest HPI annual growth in the country.   
...
Although the above spatial patterns offers detailed empirical evidence to understand the incentives for population movements between metropolitan areas within states, in light of a fundamental determinant of housing demand, which is home price fluctuations,  one question that arises in the context of economic determinants of house prices is:  What role does available economic policy instruments have played to shape the US real estate markets in the last two decades? 
...
Figure 4, presents the evolution of the FFER in the period from July 2000 to May 2023. 

**Figure 4 Monetary policy: The federal Funds Effective rate evolution July 2000  May 2023**
```{r, echo = FALSE}
...
The representation above covers 23 years of Federal Funds effective rate fluctuations. Over the span of this period, monetary policy has experienced important changes with clear implications for the housing markets. 
...
This dovish monetary policy period (low interest rates) was extended for almost two years and only started to be normalized by April 2022. 
...
**Figure 5 California Google trends score 2004-2023. Selected terms**
```{r, echo=FALSE}
# Plots  google trends.
...
We can clearly note the effect of the *sub-prime mortgage market* collapse in the population sentiment reflected as a higher volume of search for terms such as "affordable home" which indicate a tight real estate market during the second quarter of **2009**. 
...
**Figure 6 Real estate at market value and rising equity 1980-2023**
```{r, echo=FALSE}
# Plots.
## Rising equity 
## loading data 
...
As monetary policy measures were incorporated by the markets, existing home owner's home equity more than double during the last decade (from 2006Q4 to 2022Q4 with a 213% growth).

**Why there was such growth in home equity in the nation during the last decade?** 

As we have seen, special monetary policy measures took place in response to this period´s complex macroeconomic scenario, given a combined period of ultra low interest rates, reinforced by measures to induce liquidity to the markets and promote recovery such as quantitative easing^[While QE and low interest rates often go hand in hand, is worth nothing that they are distinct policy tools. QE is a specific strategy involving the purchase of assets, while low interest rates refer to the level at which the central bank sets it's benchmark interest rate. In this particular case, both policy tools were used.]. Starting from December 2008 with a Federal reserve effective rate (FFFER) = $0.16\%$ down from $5.26\%$ in July 2007 -prior to sub prime mortgage market collapse- Followed by a second ultra low FFER period that was implemented to cope with a deceleration of the economy in the presence of the global public health emergency triggered  by the COVID-19 restrictions, starting at 2020Q1 with FFER = $0.05\%$ down from FFER=$2.40 \%$ in june 2019. 

**Who benefited from this historical monetary policy scenario  characterized by ultra low interest rate?** 

Specially people who bought homes in attracting real estate markets areas with dynamic internal migration patterns, who manage to **profit** from this  **historic ultra low interest rate** period, by entering the market and securing a Fixed-rate mortgage just after the sub prime mortgage collapse, that is **between December 2008 and November 2015.**  

This movers took advantage of the economic stimulus provided by the macroeconomic environment created with a monetary policy oriented for **quantitative easing**, and dovish interest rates, a strategy  designed to lower the price of money in an effort to boost consumption and investment, among other things. For example, **home equity** for existing housing stock in the US in 2012Q1 was **$10.25$T** and it **grew to $33.78$T in 10 years**, virtually a 3X fold increase!**

In addition, as the housing market gain stem after the policy actions, investment trends across Metropolitan areas start to develop reshaping the home prices geography and creating incentives for people to relocate.   Detailed mortgage records kept by government sponsored enterprises such as Freddie Mac and Fannie Mae, allow to distinguish the type of loan granted to actual current mortgage clients, this data is important to show real estate reconfiguration in terms of housing demand. 

Considering data for 2022, nationwide **87.3%** of **Freddie Mac clients** are paying a loan for a house that is a **primary residence.** Almost **4%** of  Freddie Mac home buyers are buying a **second home**.  And **8.9%** are paying a home loan classified as **investment.** 
...
**Figure 7. Top 20 Metropolitan Areas. Investment Home loans 2022-Q1**
...
As we have shown, the implementation of historic measures to stabilize the economic environment lead to very special and not usual conditions that served as changing forces that have impacted the real estate market landscape. These factors created both, market incentives for investment opportunities as well as affordability issues for some segments of the US population, that, in the face of being price-out are reinforcing current internal migration patterns.


The following map shows the "after" scenario featuring HPI levels for 2023. 


**Figure 8b HPI by state 2023**
```{r,echo=FALSE, warning=FALSE, message=FALSE}
...
Following the implementation of macroeconomic policy with an extended period of favorable interest rates, empirical evidence suggest that the housing market have been diversifying out of the **traditional high cost regions**^[For the purposes of this investigation, traditional high cost regions refers to  California, Florida and the north eastern cost states of New York, Virginia and Maryland] of the country. 

In this regard we identify at least two important geographic patterns  I) A movement of high housing prices from the pacific west cost towards the northeastern states closer to Canadian border.  Furthermore, considering the south, two Mexican border states are showing a rapidly increasing real estate market: Arizona and Texas II) Traditionally expensive states in the north east, have been experiencing a relative cooling off in favor of emerging regions. 


Among the emerging regions, there are two particular cases that require a closer examination:


**The case of Arizona** The significant House Price Index (HPI) increase observed in Arizona, particularly in the Phoenix metropolitan statistical area, can be attributed to various real estate elements and economic features. Some factors that may explain this trend include: population growth and economic development, both factors impacting prices via a growing housing demand. 

Population Growth: Phoenix has experienced substantial population growth over the last two decades. The increasing population creates a higher demand for housing, leading to upward pressure on home prices.


Economic Development: Phoenix has been a hub for economic development, attracting industries such as technology, finance, healthcare, and manufacturing. The presence of major companies, business-friendly policies related to tax regulations, and a growing job market have contributed to the region's economic growth. This, in turn, stimulates demand for housing which exhibits a direct positive relation to price increases. The next chapter analyses this relationship in detail. 
...
Finally figure 9 below represents the evolution of house prices for US Mexico border Metropolitan statistical areas over the last four decades.  

**Figure 9 US-Mexico border (MSAs). Housing prices evolution 1975-2020**
