...
We now examenine four discernible patterns that emerge when analyzing the top 20 metropolitan areas with the highest appreciation rates as measured by the annual growth on the HPI during the last 27 years: 
...
Figure 7  below ranks metro (MSAs) by the number of loans classified as investment by Freddie Mac single family loan data for the first quarter of 2022 [@freddiemachomeSingleFamilyLoanLevel2023]. 
...
Figure 8 below shows a national perspective for the distribution of home price index at a moment before the implementation of a dovish monetary policy. 
...
As we have shown, the implementation of historic measures to stabilize the economic environment lead to very especial and not usual conditions that served as changing forces that have impacted the real state market landscape. This factors created both,market incentives for investment opportunities as well as affordability issues for some segments of the US population, that in the face of being price-out are reinforcing internal migration patterns.
...
Basic economic theory suggest that housing demand is influenced by specific demographic factors. Population growth is a paramount factor, in particular the interaction between population outflows and those incoming that define net migration shape housing demand  which in turn, given the scarcity principle,  ensures an upward signal to the pricing system. 

Although net migration flows allow to distinguish more subtle attraction elements that each geographic area has offer, it is the population movement that manifest to balance supply and demand. But why population moves, centrally another economic variable plays a key role to understand incentives to relocate or the basic decision to acquire a home. That is income, per capita income, which is directly related to the labor market dynamics in a given time and particular space. 

Finally home values by itself reinforce the demand and supply equilibrium, although this important factor can be particularly flexible when considering real state market  environment, given forces such as expectations play a pivotal role in the fluctuations of home prices creating additional complexities.

The following figure considers data on the factors discussed above, for the all Metropolitan statistical areas located at the US Mexico border States. The aim of the analysis is to capture the relationship among this important factors and distiguish patterns within the border region.   



[Figure 9 Housing market determinants: HPI, population and percapita income 2020](../speech/F9_scatter.html)
...
Finally figure 10 below represents the evolution of house prices for US Mexico border Metropolitan statistical areas. 

**Figure 10 US-Mexico border (MSAs). Housing prices evolution**
