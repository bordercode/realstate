...
# Here we use slice_max to get the top n rows of var avg_q20

top20<-df%>%
 slice_max(hpi20, n=20)%>%
  mutate(State=as.factor(ifelse(state==" CA"|state==" CA (MSAD)","California","Other state")))
  
number<-count(top20, State=="California")
## The plot.

top<-ggplot(top20, aes(hpi20, reorder(msa.x, hpi20), fill=State))+
  geom_col()+
  scale_fill_manual(values = alpha(c("dodgerblue4", "gray")))+
  labs(
...
dygraph(ts_FFER, main = "Quantitative easing")%>% 
dyRangeSelector()%>%
...
The representation above covers 23 years of interest rates fluctuations, over the span of this period, monetary policy has experienced important changes with implications for the housing markets. The first, marked here with a benchmark date in reference to the sub-prime mortgage market collapse ^(Following real state market terminology a sub-prime mortgage is defined here as higher-risk loans granted to borrowers with lower creditworthiness), an episode in US economic recent history that constitutes one of the main factors defining the so called great recession.

Prior to this period, the Federal Reserve´s monetary policy had set the FFER at $5.26\%$ which its the highest interest rate that US society has experienced between July 2007 and the moment of the writing of this work (May 2023).

Following the sub-prime mortgage market collapse, which set the stage for the beginning of the global financial crisis, the US central bank, decided to implement measures via the its key monetary policy instruments, to stimulate the economy. Given the magnitude and scope of the economic downturn, the recovery path was expected to be a long upward slope, so the FFER, was decidedly moved down to historic lows reaching  $0.07\%$ in January 2014.
...
However when quantitative easing seemed to be reversed, on January 31, 2020, Department of Health and Human Services (HHS) under the Public Health Service Act, declared a public COVID-19 health emergency. This declaration allowed the HHS to activate emergency response mechanisms, mobilize resources, and coordinate efforts to contain and mitigate the spread of the virus within the country. Among those measures, as we now Know, there were significant mobility restrictions that infringed a toll on economic activity with emphasis in the labor market. Triggering a relevant economic slowdown, that required once more, ta decesive intervention by the central bank authorities to prevent a deeper economic crisis and to stimulate a recovery!!!
