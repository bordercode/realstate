...
**Figure XX, Net migration flows from California, by state and Housing market cost**
...
**Figxxx  Main Metropolitan destinations for out of state Californian  movers 2020**
...
ca_out<-readRDS("ca_out.rds")%>%
  rename(Population=moved)%>%
  arrange(desc(Population))%>%
  mutate(acumulado=cumsum(Population ), cdf=acumulado/sum(Population))%>%
  slice_max(Population, n=78)%>%
  rename(Netflow=Population)
...
####################################
msap<-ggplot(ca_out, aes(Netflow, reorder(MSA,Netflow), label=City, text=State, fill=Netflow, ))+
geom_col()+
 scale_fill_gradient(low = "grey", high = "#FF0F0F", name="" )+
  labs(
    x = 'Net migration from California MSA',
    y = 'Destination (MSAs)')+
  theme(legend.position="bottom")+
...
ggplotly(msap, tooltip=c("text",  "City", "Netflow"))%>%
  layout(legend = list(orientation = "h", x = 0, y = -.05))

# we observe a Gini coef =.71 which reflects a highly unequal pattern of concentration of movers in a tggh   MSA set. 
```

Source: José Luis Manzanares Rivera based on data from [@uscensusbureauacsCountyMCDtoCountyMCD2023]
...
#Pop<-ponderador1%>%filter(typemove=="Cheaper")%>%group_by(City2, State2)%>%summarize(Pop=abs(sum(MOVEDNET)))

#hpigap<-ponderador1%>%filter(typemove=="Cheaper")%>%group_by(City2, State2)%>%summarize(hpigap=mean(hpigap))%>%left_join(Pop)

#Cheapermove<-ponderador1%>%
  #filter(typemove=="Cheaper")%>%mutate(sp=hpigap/hpi)%>%group_by(City2, State2)%>%summarize(savings=mean(sp))%>%left_join(hpigap)

#Cheapermove$Location <- paste(Cheapermove$City2, Cheapermove$State2, sep = ", ")

#location_MSA<-read.csv("locationsMSACheap.csv")%>%na.omit()
...
Figure xxx below shows the geographic distribution of destinations adding de HPI gap between origin and destination along with the net flow and  the housing market savings indicator which represent the percentage that the HPI gap between origin and destination, represents relative to the origin home price level. 

**Figure xxx  Net out of state migration flows from California by destination MSA, HPI gap and savings as % of origin  HPI. ACS 2016-2020**
```{r, echo=FALSE, warning=FALSE, message=FALSE}
...
cheaper<-mapview(location_MSA, map.types =  "CartoDB.DarkMatter",cex="Netflow", zcol="savings", layer.name= "Saving %",legend = TRUE,homebutton =FALSE, col.regions=brewer.pal(9, "YlOrRd"), alpha.regions = 1, aplha = 0.2)


leafem::addMouseCoordinates(cheaper)%>%
