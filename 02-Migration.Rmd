...
By examining the nexus of geographic mobility, income differentials, and racial composition, we aim to deepen our understanding of the complex interrelationships within metropolitan contexts and their implications for regional economic development with a focus on the Hispanic population. 
...
To retrieve the data, Census API endpoints calls are made using Python on jupyter notebooks environment. The visualization and data management is done with  Posit (before known as R studio) Rmarkdown integrated development environment.
...
$P_i=B07001065E$  where **B07001_065E**  is table id for data on Geographical mobility in the past year for current residence in the United States, category: Moved from different state, for $MSA_{i}$  And P, is the total population count for the 384 MSA considered in the study.  So in essence **B** measures the proportion that MSA represents within the total US MSA set. 
...
```
Source: José Luis Manzanares Rivera with data from:  [@uscensusbureauAmericanCommunitySurvey2021]
...
What about migration patterns between other racial groups. For example, where are Asians mostly moving to? What about the African american segment. We know where are Hispanics moving to but what about the White population?
...
San Jose-Sunnyvale-Santa Clara MSA, an area with world wide recognition as a hub for entrepreneurship, innovation and the technology sector. With a LQ=3.68
...
Seattle-Tacoma-Bellevue MSA, another area with increasingly important activity in the tech sector, with a labor market linked to silicon Valley companies such as Microsoft, Amazon, Facebook and Google. With an Asian concentration three times that observed by this population group at the country scale (LQ=3.36).
...
Killeen-Temple, TX, which host Fort Hood, an army complex which is among the largest United States military installations worldwide (encompasses 340 square miles (bigger than the city of Dallas, TX. employing about 37,000 military personnel) and is home to many elite military units, such as the 1st Cavalry Division, 4th Infantry Division, Command III Corps, 504th Military Intelligence Brigade, 21st Cavalry Brigade Air Combat. (LQ=5.04). 
...
Concerning the White non Hispanic population it is also of interest to note the places that are the least preferred to migrate, that is the metropolitan areas that exhibit the lowest relative concentration of migrants for this population segment.  The bottom 5 MSA by the magnitude of its location quotient LQ in this regard, lay right at the US-Mexico Border. With Laredo, Tx, being the MSA with the lowest concentration of out of state white movers, followed by the Brownsville-Harlingen, TX, MSA, Madera, CA, 	
McAllen-Edinburg-Mission, Tx and Vineland-Bridgeton, New Jersey.


Now we turn our attention to explain important housing demand side determinants, in particular we are interested in the association between personal income and Metropolitan area size as measure by its population. 
...
The following figure considers data on the factors discussed above, for all Metropolitan statistical areas located at the US Mexico border States. The aim of the analysis is to capture the relationship among this important factors and distinguish patterns within the US-Mexico border region a strategic territory that bears implications beyond the national scale.   
...
## just a proof test for correlation statistical significance ## 

##cor_test_by_state <- function(subset) {   result <- cor.test(subset$pp_inccome, subset$hpi)   return(data.frame( correlation = result$estimate,     conf_interval_low = result$conf.int[1],     conf_interval_high = result$conf.int[2],p_value = result$p.value))}

# Apply the function to each state
#cor_test_results <- by(only_border2020, only_border2020$state, cor_test_by_state)

# Convert the result to a data frame
#cor_test_df <- do.call(rbind, cor_test_results)
#print(significant_results)

# Print or view the result
#print(cor_test_df)
#significant_results <- cor_test_df$p_value < 0.05
...
Although data provides sound evidence that personal per capita income and house prices are positively correlated in California, (where we also find the most expensive metropolitan areas of the border states considering housing prices), with a statistical significant relationship^[Corr = 0.8727813, p-value = $6.106278e-09$ < $\alpha$ 0.05] degrees of correlation varies between the rest border states metropolitan areas, with not significant correlations in Arizona and New Mexico and only a weak relationship being observed by Texas MSA's^[corr=$0.45$, p-value = $2.675287e-02$, $\alpha$=0.05].

In addition, there is only a weak positive relationship between house prices as measured by the HPI and population size (population size being a proxy for housing demand), a good example of this behavior is the case for New Mexico, where relative large size metropolitan areas such as Albuquerque still exhibit affordable house prices. 
...
We found that within the subset of MSAs that are located along the US-Mexico border, 95% lay inside the third quadrant,that is they exhibit a combination of below average personal income with relatively inexpensive house prices, Whereas only one  MSA in this particular geographic region, is situated at the first quadrant, which is the San Diego-Chula vista-Carlsbad in California´s southern border. 

 
This feature denotes a very different set of economic dynamics for the local movement of people across the border, making San Diego-Chula Vista-Carlsbad a very prone area for outmigration, including cross-border migration. This is due to the real estate market price differentials combined with the economic integration that bi-national trade and manufacturing hubs have developed across the border in the Mexican metropolitan area of Tijuana.  
...
Examining major urban centers within the US-Mexico border region reveals noteworthy differentials, particularly evident in the comparison between El Paso, TX, and San Diego, CA. As the second-largest metropolitan area along the US-Mexico border, El Paso exhibits a Home Price Index (HPI) approximately 70% lower than that of San Diego, a prominent city in the transborder region. Such marked variations underscore the importance of investigating housing markets in specific locales along the US-Mexico border, with the Tijuana-San Diego corridor serving as a pertinent case study for in-depth analysis.
...
In addition, empirical evidence from the American Community Survey (ACS) internal migration flows, indicate that California is a leading state concerning negative migration flows and has been consistently experiencing negative net flows from its main Metropolitan areas during the last decade. 
...
**Figure 4. Net migration flows from California, by state and Regional price parities 2020**
...
To achieve this goal we now analyze the negative net migration flows (that is, flows in which emigration surpassed arrivals) by metropolitan area for the state of California. 
...
The classification is based on four categories to measure a variable called savings potential (SP). The savings potential implicit by the move is defined by the fraction that the HPI gap represents relative to origin housing prices level.  

For instance if  an HPI in the origin MSA is 128 and the destination MSA´s HPI  is 103, then the Saving potential implicit by the move expressed as percentage would be 25/128= 19.5 percent.
...
custom_colors <- c("#060c0f", "#0000ff", "#E21616")
...
```
Source: José Luis Manzanares Rivera using data from [@uscensusbureauacsCountyMCDtoCountyMCD2023]

Among the 19 counties that fit the criteria as origin Counties with negative net out flows of people in the state of California, the main counties for emigration (origin places) were: Los Ángeles, San Diego, Orange and San Bernardino.

Several impacts and social processes are influenced by housing market conditions and population movements across states. One key issue of social concern is homelessness, a major problem that has been escalating in the United States. This issue is particularly evident in some of the highest-income metropolitan areas of the country, highlighting an intriguing paradox. 
